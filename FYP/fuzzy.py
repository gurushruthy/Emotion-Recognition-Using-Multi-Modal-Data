...
op_hap = fuzz.trimf(x_op, [0, 0, 3])  # Scale : Neg Neu Pos
...
# ax2.plot(x_op, op_midd, 'b', linewidth=1.5, label='middle')
# ax2.plot(x_op, op_multi1, 'r', linewidth=1.5, label='middle1')
...
negators = ["not", "no", "none", "nothing", "never", "nobody", "nowhere", "neither", "nor", "hardly", "barely",
            "scarcely", "falsely"]
...
def tweet_emotion_detection():
    ...
    for twt in tweets:
        t.append(twt.full_text)
        print(twt.full_text)
    final_result = []
    for i in t:
        c = []
        c.append(i)
        '''if (i.entities.get('media', [])):
            url = media_url
        ...
        cosine_similarity = (generated_caption, preprocessed_text)
        if cosine_similarity > 0.1:
            predicted_image_emotion = predict_image_emotion( )
        else:
        ...
        ''' if predicted_image_emotion is not None:
            c.append(predicted_image_emotion)
        else:
            c.append("no image in tweet/cosine_similarity < 0.1")
